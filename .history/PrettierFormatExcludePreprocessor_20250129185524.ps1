...
foreach ($file in $files) {
	if (($file.Extension -eq '.css' -and (Contains-Preprocessor -filePath $file.FullName -eq $true)) -or $file.Extension -eq '.gitignore') {
...
if ($confirmation -eq 'y' -or $confirmation -eq 'Y') {
	...
	foreach ($file in $filesToFormat) {
			...
			$jobs += Start-Job -ScriptBlock { param($filePath) npx prettier --write $filePath } -ArgumentList $file
	}
