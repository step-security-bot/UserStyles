...
function calculateSpeed() {
	...
	if (distance > 0 && time > 0) {
		...
		document.getElementById(
			'speedResult',
		).innerText = `Speed: ${speed} km/h`;
	} else {
		alert(
			'Please enter valid values for distance and time.',
		);
	}
}
...
function calculatePower() {
	...
	const speed = parseFloat(
		document.getElementById('cyclistSpeed').value,
	);

	if (weight > 0 && speed > 0) {
		const power = (weight * speed * 0.2).toFixed(
			2,
		); // Simplified power calculation
		document.getElementById(
			'powerResult',
		).innerText = `Power: ${power} Watts`;
	} else {
		alert(
			'Please enter valid values for weight and speed.',
		);
	}
}
...
function calculateCalories() {
	const weight = parseFloat(
		document.getElementById('weightCalories')
			.value,
	);
	...
	if (weight > 0 && duration > 0) {
		...
		alert(
			'Please enter valid values for weight and duration.',
		);
	}
}

function calculateCadence() {
	const revolutions = parseFloat(
		document.getElementById('revolutions').value,
	);
	const time = parseFloat(
		document.getElementById('timeCadence').value,
	);

	if (revolutions > 0 && time > 0) {
		const cadence = (revolutions / time).toFixed(
			2,
		);
		document.getElementById(
			'cadenceResult',
		...
		alert(
			'Please enter valid values for revolutions and time.',
		);
	}
}
...
function calculateDistance() {
	...
	const speed = parseFloat(
		document.getElementById('speedDistance')
			.value,
	);
	...
	if (time > 0 && speed > 0) {
		...
		alert(
			'Please enter valid values for time and speed.',
		);
	}
}
...
function calculateAverageSpeed() {
	...
	if (totalDistance > 0 && totalTime > 0) {
		...
		alert(
			'Please enter valid values for total distance and total time.',
		);
	}
}
...
function calculateClimbingSpeed() {
	const distance = parseFloat(
		document.getElementById('climbDistance')
			.value,
	);
	...
	const power = parseFloat(
		document.getElementById('powerClimbing')
			.value,
	);
	...
	if (distance > 0 && gradient > 0 && power > 0) {
		const speed = (
			power /
			(gradient * distance)
		).toFixed(2); // Simplified calculation
		document.getElementById(
			'climbingSpeedResult',
		).innerText = `Climbing Speed: ${speed} km/h`;
	} else {
		alert(
			'Please enter valid values for distance, gradient, and power.',
		);
	}
}

function calculateGearRatio() {
	const chainring = parseFloat(
		document.getElementById('chainring').value,
	);
	const cog = parseFloat(
	...
	if (chainring > 0 && cog > 0) {
		...
		document.getElementById(
			'gearRatioResult',
		).innerText = `Gear Ratio: ${ratio}`;
	} else {
		alert(
			'Please enter valid values for chainring and cog.',
		);
	}
}
...
function calculateHeartRateZones() {
	...
	if (maxHeartRate > 0) {
		...
		alert(
			'Please enter a valid value for max heart rate.',
		);
	}
}
...
function estimateVO2Max() {
	const weight = parseFloat(
		document.getElementById('vo2MaxWeight').value,
	);
	const speed = parseFloat(
		document.getElementById('vo2MaxSpeed').value,
	);
...
function calculateWeightImpact() {
	const riderWeight = parseFloat(
		document.getElementById('riderWeight').value,
	);
	const bikeWeight = parseFloat(
		document.getElementById('bikeWeight').value,
	);
	...
	if (
		...
		const impact =
			((riderWeight + bikeWeight) * slope) / 100; // Simplified calculation
		document.getElementById(
			'weightImpactResult',
		).innerText =
...
function calculateWindResistance() {
	...
	if (windSpeed > 0 && cyclingSpeed > 0) {
		const resistance = (
			cyclingSpeed + windSpeed
		).toFixed(2); // Simplified calculation
		document.getElementById(
			'windResistanceResult',
		).innerText =
			`Wind Resistance: ${resistance} km/h`;
	} else {
		alert(
			'Please enter valid values for wind speed and cycling speed.',
		);
	}
}
...
function calculateRollingResistance() {
	const rollingCoefficient = parseFloat(
		document.getElementById('rollingCoefficient')
			.value,
	);
	const weight = parseFloat(
		document.getElementById('weightRolling')
			.value,
	);

	if (rollingCoefficient > 0 && weight > 0) {
		const resistance = (
			rollingCoefficient * weight
		).toFixed(2);
		document.getElementById(
			'rollingResistanceResult',
		).innerText =
...
function calculateCadenceAndGear() {
	const cadence = parseFloat(
		document.getElementById('cadenceGear').value,
	);
	const wheelDiameter = parseFloat(
		document.getElementById('wheelDiameter')
			.value,
	);
	...
	if (cadence > 0 && wheelDiameter > 0) {
		...
		alert(
			'Please enter valid values for cadence and wheel diameter.',
		);
	}
}
