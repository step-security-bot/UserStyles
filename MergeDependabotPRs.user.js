...
(async function () {
  ...
  async function initialize() {
    ...
    try {
      ...
      alert(
        "Failed to retrieve and decrypt token. Please check the console for more details."
      );
      return;
    }
    ...
    if (!token) {
      ...
      if (token) {
        try {
          ...
          alert(
            "Failed to encrypt and store token. Please check the console for more details."
          );
          return;
        }
      } else {
      ...
      GM_xmlhttpRequest({
        ...
        onload: function (response) {
          if (response.status === 200) {
            ...
            reject(
              new Error(
                `Failed to fetch repositories: ${response.responseText}`
              )
            );
          }
        },
      ...
      GM_xmlhttpRequest({
        ...
        onload: function (response) {
          if (response.status === 200) {
            ...
            reject(
              new Error(
                `Failed to fetch pull requests: ${response.responseText}`
              )
            );
          }
        },
      ...
      try {
        ...
        for (const repo of repos) {
          if (repo.archived) {
            updateStatusElement(
              statusElement,
              `Skipping archived repo: ${repo.name}`
            );
            continue;
          }
          updateStatusElement(
            statusElement,
            `Fetching PRs for repo: ${repo.name}`
          );
        ...
        if (allPRs.length > 0) {
          ...
          updateStatusElement(
            statusElement,
            "No Dependabot PRs found to merge."
          );
          displayNoPRsMessage();
        }
  ...
  function getStatusElement() {
    ...
    if (!statusElement) {
      ...
      statusElement.style =
        "position: fixed; bottom: 70px; right: 10px; z-index: 1000; background-color: #79e4f2; padding: 10px; border: 1px solid #ccc;";
  ...
  function displayPRSelection(prs, username, token) {
    ...
    mergeSelectedButton.addEventListener("click", async () => {
      const selectedPRs = Array.from(
        prList.querySelectorAll("input:checked")
      ).map((input) => prs.find((pr) => pr.number == input.value));
