...
def get_git_repo_url():
    try:
        try:
            ...
            url = (
                result.stdout.strip()
            )  # Get the URL from the output and strip whitespace
    ...
    ) as e:  # Handle errors from the Git command
        logging.error(
...
try:
    ...
    if not isinstance(IGNORE_LIST, list) or not all(
        isinstance(item, str) for item in IGNORE_LIST
    ):
    ...
    if DEFAULT_COLOR_SOURCE not in ["random", "list"]:
        raise ValueError(
            "DEFAULT_COLOR_SOURCE must be either 'random' or 'list'."
        )

    if not isinstance(
        DEFAULT_COLOR_LIST, list
    ) or not all(  # Check if the color list is a list of hex color strings
    ...
    if DEFAULT_COLOR_RANGE is not None:  # Check if the color range is not None
        if (  # Check if the color range is invalid
            not isinstance(
                DEFAULT_COLOR_RANGE, tuple
            )  # Check if the color range is a tuple
            or len(DEFAULT_COLOR_RANGE)
            != 2  # Check if the color range has two elements
            or not all(  # Check if the color range elements are valid hex color strings
                ...
                and color.startswith(
                    "#"
                )  # Check if the color starts with a hash symbol
    ...
    ):  # Check if the chunk size is a positive integer
        raise ValueError(
            "CHUNK_SIZE must be a positive integer."
        )  # Raise an error if the chunk size is invalid
    ...
    if (
        ...
        )  # Check if the small desktop viewport size is a positive integer
        or VIEWPORT_SMALL_DESKTOP
        <= 0  # Check if the small desktop viewport size is a positive integer
    ):
        raise ValueError(
    ...
    ):  # Check if the root margin for large desktops is a string
        raise ValueError(
    ...
    ):  # Check if the root margin for small desktops is a string
        raise ValueError(
    ...
    ):  # Check if the root margin for tablets is a string
        raise ValueError(
    ...
    ):  # Check if the root margin for mobile devices is a string
        raise ValueError(
    ...
    ) or not all(  # Check if the file categories are a list of dictionaries
    ...
    ):  # Check if the repository root header is a string
        raise ValueError(
    ...
    if not isinstance(HEADER_TEXT, str):  # Check if the header text is a string
        raise ValueError(
            "HEADER_TEXT must be a string."
        )  # Raise an error if the header text is invalid

    if not isinstance(
        INTRO_TEXT, str
    ):  # Check if the introductory text is a string
        raise ValueError(
            "INTRO_TEXT must be a string."
        )  # Raise an error if the introductory text is invalid
    ...
    if (
        not EXCLUDE_BLACKS_THRESHOLD.startswith(
            "#"
        )  # Check if it starts with '#'
        or len(EXCLUDE_BLACKS_THRESHOLD) != 7  # Check if it has 7 characters
    ):
        raise ValueError(
            "EXCLUDE_BLACKS_THRESHOLD must be a hex color code in the format #RRGGBB."
        )

    try:
        int(
            EXCLUDE_BLACKS_THRESHOLD[1:], 16
        )  # Try to convert the hex code to an integer
    except ValueError:
        raise ValueError(
            "EXCLUDE_BLACKS_THRESHOLD must be a valid hex color code."
        )
    ...
    if (
        ...
        raise ValueError(
            f"Invalid log level: {LOG_LEVEL_SETTING}. Valid log levels are: {', '.join(VALID_LOG_LEVELS)}"
        )
except ValueError as e:  # Handle errors
    logging.error(
        f"\033[1;31mConfiguration validation error: {e}\033[0m"
    )  # Log the error
...
def sort_color_range(color_range):
    ...
    try:
        return tuple(  # Return the sorted tuple of hex color strings
            sorted(
                color_range, key=lambda color: int(color[1:], 16)
            )  # Sorts the color ranged from low to high
        )
    except ValueError as e:  # Handle errors
        logging.error(
            f"\033[1;31mError sorting color range: {e}\033[0m"  # Log the error
        )
...
def should_ignore(  # Determine if a path should be ignored based on the ignore list
    ...
    try:  # Try to determine if the path should be ignored
        ...
        for (
            ignore_item
        ) in ignore_list:  # Iterate over the items in the ignore list
            if ignore_item in normalized_path.split(
                os.sep
            ):  # Check if the ignore item is in the normalized path
        ...
        for root, dirs, files in os.walk(directory):
            ...
            for file in files:  # Iterate over the files
                ...
                if not ignore_file:  # Check if the file should not be ignored
                    ...
                    file_list.add(  # Add the normalized path to the set
                        normalized_path  # Normalized path to add
                    )
...
def is_dark_color(hex_color):  # Determine if a hex color is dark
    ...
    try:  # Try to determine if the color is dark
        luminance = calculate_luminance(  # Calculate the luminance of the color
            hex_color  # Hex color code
        )
        return (  # Return True if the luminance is below the threshold
            luminance
            < DARK_COLOR_LUMINANCE_THRESHOLD  # Check if the luminance is below the threshold
        )
...
def is_bright_color(hex_color):  # Determine if a hex color is bright
    ...
    try:  # Try to determine if the color is bright
        ...
        return (
            luminance
            > BRIGHT_COLOR_LUMINANCE_THRESHOLD  # Check if the luminance is above the threshold
    ...
    try:
        ...
        for _ in range(  # Iterate over the maximum attempts
            ...
            if color_range:  # Check if a color range is specified
                ...
                r = random.randint(  # Generate a random value for the red component
                    r_min, r_max
                )
                g = random.randint(  # Generate a random value for the green component
                    g_min, g_max
                )
                b = random.randint(  # Generate a random value for the blue component
                    b_min, b_max
                )
...
def should_exclude_color(color):  # Determine if a color should be excluded
    ...
    try:
        ...
        return (
            False  # Do not exclude the color if none of the conditions are met
        )
    except Exception as e:
        logging.error(
            f"\033[1;31mError in should_exclude_color function: {e}\033[0m"
        )
        return False
...
def generate_file_list_with_links(
    ...
    file_list_html = defaultdict(
        list
    )  # Use defaultdict to group files by folder
    try:  # Try to generate the file list with links
        for file in file_list:  # Iterate over the file list
            ...
            for category in FILE_CATEGORIES:  # Iterate over the file categories
                if file.endswith(
                    category["ext"]
                ):  # Check if the file ends with the category extension
    ...
    try:  # Try to generate the sorted HTML
        ...
        if root_files:  # Check if there are root files
            sorted_html.append(
                f"<li><h2>{REPO_ROOT_HEADER}</h2></li>"
            )  # Add the root header
            sorted_html.extend(
        ...
        for category in FILE_CATEGORIES:  # Iterate over the file categories
            if category["files"]:  # Check if the category has files
                sorted_html.append(
                    f'<li><h2>{category["name"]}</h2></li>'
                )  # Add the category header
                sorted_html.extend(
                    sorted(category["files"])
                )  # Add the sorted category files

        for folder in sorted(file_list_html):  # Iterate over the folders
            sorted_html.append(
                f"<li><h2>{folder}</h2></li>"
            )  # Add the folder header
            sorted_html.extend(
        ...
        logging.error(
            f"\033[1;31mError generating sorted HTML: {e}\033[0m"
        )  # Log the error
...
def sort_files_by_extension(files):  # Sort files by their extensions
    ...
    try:  # Try to sort the files by their extensions
        return sorted(
            files, key=lambda x: os.path.splitext(x)[1]
        )  # Sort the files by their extensions
    except Exception as e:  # Handle errors
        logging.error(
            f"\033[1;31mError sorting files by extension: {e}\033[0m"
        )  # Log the error
        return files  # Return the original list of files


def save_file_list(
    file_list_html, output_file
):  # Save the file list to an output file
...
def split_into_chunks(file_list_html, chunk_size):
    ...
    try:
        ...
        logging.error(
            f"\033[1;31mError splitting file list into chunks: {e}\033[0m"
        )
        return []
...
def write_html_header(f):  # Write the HTML header to the file
    ...
    try:
        ...
        logging.error(
            f"\033[1;31mError writing HTML header: {e}\033[0m"
        )  # Log the error
...
def write_lazyload_placeholders(f, file_list_chunks):
    ...
    try:
        ...
        logging.error(
            f"\033[1;31mError writing lazyload placeholders: {e}\033[0m"
        )


def write_lazyload_script(
    f, file_list_chunks
):  # Write the lazy loading script to the file
    ...
    try:  # Try to write the lazy loading script to the file
        ...
        for i in range(
            len(file_list_chunks)
        ):  # Iterate over the file list chunks
            f.write(
        ...
        logging.error(
            f"\033[1;31mError writing lazyload script: {e}\033[0m"
        )  # Log the error
...
if __name__ == "__main__":  # Main entry point of the script
    ...
    if args.ignore_list:  # Check if ignore list is provided
        if args.overwrite_ignore_list:  # Check if the overwrite flag is set
            ...
            IGNORE_LIST.extend(
                args.ignore_list
            )  # Extend the default ignore list with the additional ones
    ...
    if args.color_range:  # Check if the color range argument is provided
        try:  # Try to sort the color range
            if not all(  # Check if all color codes are valid
                color.startswith(
                    "#"
                )  # Check if the color code starts with a hash symbol
                and len(color)
                == 7  # Check if the color code has the correct length
                and all(
            ...
            args.color_range = sort_color_range(
                args.color_range
            )  # Sort the color range
        except ValueError as e:  # Handle errors
    ...
    DEFAULT_COLOR_RANGE = (
        args.color_range
    )  # Set the default color range sorted from lowest to highest
    EXCLUDE_DARK_COLORS = (
        args.exclude_dark_colors
    )  # Set the exclusion of dark colors
    ...
    ROOT_MARGIN_LARGE_DESKTOP = (
        args.root_margin_large_desktop
    )  # Set the root margin for the large desktop viewport
    ROOT_MARGIN_SMALL_DESKTOP = (
        args.root_margin_small_desktop
    )  # Set the root margin for small desktops
    ROOT_MARGIN_TABLET = (
        args.root_margin_tablet
    )  # Set the root margin for tablets
    ROOT_MARGIN_MOBILE = (
    ...
    logging.info(
        f"\033[1;32mIGNORE_LIST is set to:\033[0m {IGNORE_LIST}"
    )  # Log the ignore list
    logging.info(
    ...
    file_list_html = (
        generate_file_list_with_links(  # Generate the file list with links
