...
(function (global, factory) {
	...
	var ie_version =
		...
		(ie_upto10
			...
			: +(edge || ie_11up)[1]);
	var webkit =
		!edge && /WebKit\//.test(userAgent);
	var qtwebkit =
		webkit && /Qt\/\d+\.\d+/.test(userAgent);
	var chrome =
		!edge && /Chrome\/(\d+)/.exec(userAgent);
			...
			for (var i$6 = 0; i$6 < len; ++i$6) {
				if (isNeutral.test(types[i$6])) {
					...
					for (
						...
						isNeutral.test(types[end$1]);
						++end$1
					) {}
					var before =
						(i$6 ? types[i$6 - 1] : outerType) ==
						'L';
					var after =
						(end$1 < len
							? types[end$1]
	...
	function signalCursorActivity(cm) {
		var arr =
			cm._handlers && cm._handlers.cursorActivity;
		if (!arr) {
			return;
		}
		var set =
	...
	StringStream.prototype.eatWhile = function (
		match,
	) {
		var start = this.pos;
	...
	StringStream.prototype.indentation =
		function () {
			return (
				countColumn(
					this.string,
	...
	StringStream.prototype.hideFirstChars =
		function (n, inner) {
			this.lineStart += n;
			try {
				return inner();
	...
	StringStream.prototype.lookAhead = function (
		n,
	) {
	...
	function isLine(doc, l) {
		return (
			l >= doc.first && l < doc.first + doc.size
		);
	}
	...
	function clipPos(doc, pos) {
		...
		return clipToLen(
			pos,
			getLine(doc, pos.line).text.length,
		);
	}
	...
	Context.prototype.baseToken = function (n) {
		...
		var type =
			this.baseTokens[this.baseTokenPos + 1];
		return {
			type:
				type &&
	...
	function getLineStyles(
		...
		if (
			!line.styles ||
			line.styles[0] != cm.state.modeGen
		) {
			var context = getContextBefore(
				cm,
				lineNo(line),
			);
			var resetState =
	...
	function getContextBefore(cm, n, precise) {
		...
		var saved =
			start > doc.first &&
			getLine(doc, start - 1).stateAfter;
		var context = saved
	...
	function findStartLine(cm, n, precise) {
		...
		var lim = precise
			? -1
			: n - (cm.doc.mode.innerMode ? 1000 : 100);
	...
	function stretchSpansOverChange(doc, change) {
		...
		var first = markedSpansBefore(
			oldFirst,
			startCh,
			isInsert,
		);
		var last = markedSpansAfter(
			oldLast,
			endCh,
			isInsert,
		);
	...
	function compareCollapsedMarkers(a, b) {
		...
		var aPos = a.find(),
			bPos = b.find();
		var fromCmp =
			cmp(aPos.from, bPos.from) ||
			extraLeft(a) - extraLeft(b);
		if (fromCmp) {
			return -fromCmp;
		}
		var toCmp =
			cmp(aPos.to, bPos.to) ||
			extraRight(a) - extraRight(b);
		if (toCmp) {
			return toCmp;
		}
		return b.id - a.id;
	...
	function collapsedSpanAtSide(line, start) {
		var sps =
				sawCollapsedSpans && line.markedSpans,
			found;
		if (sps) {
			for (
				var sp = void 0, i = 0;
	...
	function heightAtLine(lineObj) {
		...
		for (
			var p = chunk.parent;
			p;
			chunk = p, p = chunk.parent
		) {
			for (
				var i$1 = 0;
				i$1 < p.children.length;
				++i$1
			) {
	...
	function buildLineContent(cm, lineView) {
		...
		for (
			var i = 0;
			i <=
			(lineView.rest ? lineView.rest.length : 0);
			i++
		) {
			var line = i
					? lineView.rest[i - 1]
	...
	function buildToken(
		...
		if (!special.test(text)) {
			...
			while (true) {
				...
				if (m[0] == '\t') {
					var tabSize =
							builder.cm.options.tabSize,
						tabWidth =
							tabSize - (builder.col % tabSize);
		...
		builder.trailingSpace =
			displayText.charCodeAt(text.length - 1) ==
			32;
		if (
			style ||
			startStyle ||
	...
	function updateLineGutter(
		...
		if (cm.options.lineNumbers || markers) {
			...
			if (markers) {
				for (
					var k = 0;
					k < cm.display.gutterSpecs.length;
					++k
				) {
					var id =
							cm.display.gutterSpecs[k].className,
						found =
	...
	function updateLineWidgets(cm, lineView, dims) {
		...
		var isWidget = classTest(
			'CodeMirror-linewidget',
		);
		for (
			var node = lineView.node.firstChild,
				next = void 0;
			node;
			node = next
		) {
	...
	function widgetHeight(widget) {
		...
		return (widget.height =
			widget.node.parentNode.offsetHeight);
	}
	...
	function scrollGap(cm) {
		return (
			scrollerGap - cm.display.nativeBarWidth
		);
	}
	...
	function updateExternalMeasurement(cm, line) {
		...
		var view = (cm.display.externalMeasured =
			new LineView(cm.doc, line, lineN));
		view.lineN = lineN;
		var built = (view.built = buildLineContent(
			cm,
			view,
		));
		view.text = built.pre;
		removeChildrenAndAdd(
			cm.display.lineMeasure,
			built.pre,
		);
		return view;
	}
	...
	function findViewForLine(cm, lineN) {
		if (
			lineN >= cm.display.viewFrom &&
			lineN < cm.display.viewTo
		) {
			return cm.display.view[
				findViewIndex(cm, lineN)
			];
		}
		var ext = cm.display.externalMeasured;
		if (
	...
	function getUsefulRect(rects, bias) {
		...
		if (bias == 'left') {
			...
			for (
				...
				if (
					(rect = rects[i$1]).left != rect.right
				) {
					break;
				}
			}
	...
	function measureCharInner(
		...
		var place = nodeAndOffsetInLineMap(
			prepared.map,
			ch,
			bias,
		);
		...
		if (node.nodeType == 3) {
			...
			if (ie && ie_version < 11) {
				rect = maybeUpdateRectForZooming(
					cm.display.measure,
					rect,
				);
			}
		...
		if (
			...
			var rSpan =
				node.parentNode.getClientRects()[0];
			if (rSpan) {
				rect = {
					left: rSpan.left,
					right:
						rSpan.left + charWidth(cm.display),
					top: rSpan.top,
					bottom: rSpan.bottom,
				};
			} else {
	...
	function intoCoordSystem(
		...
		if (
			...
			yOff +=
				lOff.top +
				(context == 'window' ? 0 : pageScrollY());
			var xOff =
				lOff.left +
				(context == 'window' ? 0 : pageScrollX());
			rect.left += xOff;
			rect.right += xOff;
		}
		rect.top += yOff;
	...
	function cursorCoords(
		...
		lineObj =
			lineObj || getLine(cm.doc, pos.line);
		if (!preparedMeasure) {
			preparedMeasure = prepareMeasureForLine(
				cm,
				lineObj,
			);
		}
	...
	function coordsCharInner(
		...
		var preparedMeasure = prepareMeasureForLine(
			cm,
			lineObj,
		);
	...
	function getDimensions(cm) {
		...
		for (
			...
			var id =
				cm.display.gutterSpecs[i].className;
			left[id] =
	...
	function posFromMouse(cm, e, liberal, forRect) {
		...
		if (
			forRect &&
			coords.xRel > 0 &&
			(line = getLine(cm.doc, coords.line).text)
				.length == coords.ch
		) {
			var colDiff =
				countColumn(
	...
	function regLineChange(cm, line, type) {
		...
		if (
			line < display.viewFrom ||
			line >= display.viewTo
		) {
			return;
		}
		var lineView =
			display.view[findViewIndex(cm, line)];
		if (lineView.node == null) {
			return;
		}
		var arr =
			lineView.changes || (lineView.changes = []);
	...
	function viewCuttingPoint(cm, oldN, newN, dir) {
		...
		while (visualLineNo(cm.doc, newN) != newN) {
			if (
				index == (dir < 0 ? 0 : view.length - 1)
			) {
				return null;
			}
			newN +=
				dir *
				view[index - (dir < 0 ? 1 : 0)].size;
			index += dir;
		}
		return { index: index, lineN: newN };
	}
	...
	function prepareSelection(cm, primary) {
		...
		for (
			...
			if (customCursor) {
				...
				collapsed ||
				cm.options.showCursorWhenSelecting
			) {
				drawSelectionCursor(
					cm,
					range.head,
	...
	function drawSelectionCursor(cm, head, output) {
		...
		if (pos.other) {
			...
			otherCursor.style.left =
				pos.other.left + 'px';
			otherCursor.style.top =
				pos.other.top + 'px';
			otherCursor.style.height =
				(pos.other.bottom - pos.other.top) *
					0.85 +
				'px';
		}
	}
	...
	function drawSelectionRange(cm, range, output) {
		...
		function drawForLine(line, fromArg, toArg) {
			...
			function wrapX(pos, dir, side) {
				var extent = wrappedLineExtentChar(
					cm,
					lineObj,
					null,
					pos,
			...
			iterateBidiSections(
				...
				function (from, to, dir, i) {
					...
					if (toPos.top - fromPos.top <= 3) {
						// Single line
						var openLeft =
							(docLTR ? openStart : openEnd) &&
							first;
						var openRight =
							(docLTR ? openEnd : openStart) &&
							last;
						var left = openLeft
							? leftSide
	...
	function restartBlink(cm) {
		...
		if (cm.options.cursorBlinkRate > 0) {
			display.blinker = setInterval(function () {
				...
				display.cursorDiv.style.visibility = (on =
					!on)
					? ''
					: 'hidden';
	...
	function updateHeightsInViewport(cm) {
		...
		var oldHeight =
			display.lineDiv.getBoundingClientRect().top;
		var mustScroll = 0;
		for (
			var i = 0;
			...
			if (width > cm.display.sizerWidth) {
				var chWidth = Math.ceil(
					width / charWidth(cm.display),
				);
	...
	function maybeScrollWindow(cm, rect) {
		if (
			signalDOMEvent(cm, 'scrollCursorIntoView')
		) {
			return;
		}
	...
	function scrollPosIntoView(
		...
		for (var limit = 0; limit < 5; limit++) {
			...
			if (scrollPos.scrollTop != null) {
				...
				if (
					Math.abs(cm.doc.scrollTop - startTop) >
					1
				) {
					changed = true;
				}
	...
	function calculateScrollPos(cm, rect) {
		...
		var docBottom =
			cm.doc.height + paddingVert(display);
		var atTop = rect.top < snapMargin,
			atBottom =
				rect.bottom > docBottom - snapMargin;
		if (rect.top < screentop) {
			result.scrollTop = atTop ? 0 : rect.top;
		} else if (rect.bottom > screentop + screen) {
			var newTop = Math.min(
				rect.top,
				(atBottom ? docBottom : rect.bottom) -
					screen,
			);
			if (newTop != screentop) {
	...
	function scrollToCoordsRange(
		...
		var sPos = calculateScrollPos(cm, {
			...
			bottom:
				Math.max(from.bottom, to.bottom) + margin,
		});
		scrollToCoords(
			cm,
			sPos.scrollLeft,
	...
	function setScrollLeft(
		...
		if (
			(isScroller
				...
				: Math.abs(cm.doc.scrollLeft - val) <
					2) &&
			!forceScroll
		) {
			return;
	...
	function measureForScrollbars(cm) {
		...
		var docH = Math.round(
			cm.doc.height + paddingVert(cm.display),
		);
		return {
			...
			barLeft: cm.options.fixedGutter
				? gutterW
				: 0,
			docHeight: docH,
			scrollHeight:
				docH + scrollGap(cm) + d.barHeight,
			nativeBarWidth: d.nativeBarWidth,
			gutterWidth: gutterW,
		};
	}

	var NativeScrollbars = function (
		place,
		scroll,
		cm,
	) {
	...
	NativeScrollbars.prototype.update = function (
		measure,
	) {
		var needsH =
			measure.scrollWidth >
			measure.clientWidth + 1;
		var needsV =
			measure.scrollHeight >
			measure.clientHeight + 1;
		var sWidth = measure.nativeBarWidth;

		if (needsV) {
			this.vert.style.display = 'block';
			this.vert.style.bottom = needsH
				? sWidth + 'px'
				: '0';
			var totalHeight =
				measure.viewHeight -
		...
		if (needsH) {
			...
			this.horiz.style.right = needsV
				? sWidth + 'px'
				: '0';
			this.horiz.style.left =
				measure.barLeft + 'px';
			var totalWidth =
				measure.viewWidth -
				measure.barLeft -
	...
	NativeScrollbars.prototype.setScrollLeft =
		function (pos) {
	...
	NativeScrollbars.prototype.setScrollTop =
		function (pos) {
	...
	NativeScrollbars.prototype.zeroWidthHack =
		function () {
			var w =
				mac && !mac_geMountainLion
					? '12px'
					: '18px';
			this.horiz.style.height =
				this.vert.style.width = w;
			this.horiz.style.pointerEvents =
				this.vert.style.pointerEvents = 'none';
	...
	function endOperation_finish(op) {
		...
		if (
			...
			display.wheelStartX = display.wheelStartY =
				null;
		}
	...
	function highlightWorker(cm) {
		var doc = cm.doc;
		if (
			doc.highlightFrontier >= cm.display.viewTo
		) {
			return;
		}
	...
	DisplayUpdate.prototype.signal = function (
		emitter,
		type,
	) {
	...
	function maybeClipScrollbars(cm) {
		...
		if (
			...
			display.nativeBarWidth =
				display.scroller.offsetWidth -
				display.scroller.clientWidth;
			display.heightForcer.style.height =
				scrollGap(cm) + 'px';
	...
	function patchDisplay(
		...
		function rm(node) {
			...
			if (
				webkit &&
				mac &&
				cm.display.currentWheelTarget == node
			) {
		...
		for (var i = 0; i < view.length; i++) {
			...
			if (lineView.hidden);
				...
				if (lineView.changes) {
					if (
						indexOf(lineView.changes, 'gutter') >
						-1
					) {
						updateNumber = false;
					}
			...
			if (!view[i].hidden) {
				if (cm.options.fixedGutter) {
					...
					if (view[i].gutterBackground) {
						view[i].gutterBackground.style.left =
							left;
					}
				}
	...
	function renderGutters(display) {
		...
		for (var i = 0; i < specs.length; ++i) {
			...
			if (className == 'CodeMirror-linenumbers') {
				...
				gElt.style.width =
					(display.lineNumWidth || 1) + 'px';
			}
		}
		gutters.style.display = specs.length
			? ''
			: 'none';
	...
	function Display(place, doc, input, options) {
		...
		d.reportedViewFrom = d.reportedViewTo =
			doc.first;
		...
		if (
			...
			if (dy && canScrollY) {
				updateScrollTop(
					...
					Math.max(
						0,
						scroll.scrollTop + dy * pixelsPerUnit,
					),
				);
			}
			setScrollLeft(
				...
				Math.max(
					0,
					scroll.scrollLeft + dx * pixelsPerUnit,
				),
			);
		...
		if (wheelSamples < 20 && e.deltaMode !== 0) {
			if (display.wheelStartX == null) {
				...
				setTimeout(function () {
					...
					display.wheelStartX =
						display.wheelStartY = null;
					if (!sample) {
						return;
					}
					wheelPixelsPerUnit =
						(wheelPixelsPerUnit * wheelSamples +
							sample) /
						(wheelSamples + 1);
					++wheelSamples;
				}, 200);
	...
	Selection.prototype.somethingSelected =
		function () {
			for (
				var i = 0;
	...
	Selection.prototype.contains = function (
		pos,
		end,
	) {
		if (!end) {
	...
	function normalizeSelection(
		...
		var mayTouch =
			cm && cm.options.selectionsMayTouch;
	...
	function resetModeState(cm) {
		...
		cm.doc.modeFrontier =
			cm.doc.highlightFrontier = cm.doc.first;
		startWorker(cm, 100);
		cm.state.modeGen++;
		if (cm.curOp) {
			regChange(cm);
	...
	function isWholeLineUpdate(doc, change) {
		return (
			...
			lst(change.text) == '' &&
			(!doc.cm ||
				doc.cm.options.wholeLineUpdateBefore)
		);
	}
	...
	function addChangeToHistory(
		...
		hist.lastOrigin = hist.lastSelOrigin =
			change.origin;
	...
	function mergeOldSpans(doc, change) {
		...
		var stretched = stretchSpansOverChange(
			doc,
			change,
		);
		if (!old) {
	...
	function extendSelection(
		...
		if (extend == null) {
			extend =
				doc.cm &&
				(doc.cm.display.shift || doc.extend);
		}
		setSelection(
			doc,
			new Selection(
	...
	function setSelectionNoUndo(doc, sel, options) {
		...
		setSelectionInner(
			doc,
			skipAtomicInSelection(doc, sel, bias, true),
		);
	...
	function makeChangeFromHistory(
		...
		var suppress =
			doc.cm && doc.cm.state.suppressEdits;
		...
		for (
			var i$1 = event.changes.length - 1;
			i$1 >= 0;
			--i$1
		) {
	...
	function shiftDoc(doc, distance) {
		...
		doc.sel = new Selection(
			map(doc.sel.ranges, function (range) {
				return new Range(
					Pos(
						range.anchor.line + distance,
						range.anchor.ch,
					),
					Pos(
						range.head.line + distance,
						range.head.ch,
					),
				);
			}),
	...
	function makeChangeSingleDoc(
		...
		if (change.from.line < doc.first) {
			var shift =
				change.text.length -
				1 -
				(doc.first - change.from.line);
			shiftDoc(doc, shift);
			change = {
				from: Pos(doc.first, 0),
				to: Pos(
					change.to.line + shift,
					change.to.ch,
				),
		...
		if (
			doc.cantEdit &&
			skipAtomic(doc, Pos(doc.firstLine(), 0))
		) {
			doc.cantEdit = false;
		}
	}
	...
	function makeChangeSingleDocInEditor(
		...
		if (
			doc.sel.contains(change.from, change.to) >
			-1
		) {
	...
	BranchChunk.prototype = {
		...
		maybeSpill: function () {
			...
			var me = this;
			do {
				var spilled = me.children.splice(
					me.children.length - 5,
					5,
				);
		...
		changeLine(
			...
			function (line) {
				...
				if (cm && !lineIsHidden(doc, line)) {
					var aboveVisible =
						heightAtLine(line) < doc.scrollTop;
					updateLineHeight(
						line,
	...
	TextMarker.prototype.attachLine = function (
		line,
	) {
	...
	TextMarker.prototype.detachLine = function (
		line,
	) {
		this.lines.splice(
			indexOf(this.lines, line),
			1,
		);
		if (!this.lines.length && this.doc.cm) {
			var op = this.doc.cm.curOp;
	...
	function detachSharedMarkers(markers) {
		var loop = function (i) {
			...
			for (
				var j = 0;
				j < marker.markers.length;
				j++
			) {
				var subMarker = marker.markers[j];
				if (
					indexOf(linked, subMarker.doc) == -1
				) {
		...
		BranchChunk.prototype,
		{
			...
			isClean: function (gen) {
				return (
					...
					(gen || this.cleanGeneration)
				);
			},

			getHistory: function () {
				return {
					done: copyHistoryArray(
						this.history.done,
					),
					undone: copyHistoryArray(
						this.history.undone,
					),
				};
			},
				...
				var realOpts = {
					replacedWith:
						...
						(options.nodeType == null
							...
							: options),
					insertLeft:
						options && options.insertLeft,
					clearWhenEmpty: false,
					shared: options && options.shared,
					handleMouseEvents:
						options && options.handleMouseEvents,
				};
				...
				if (spans) {
					for (var i = 0; i < spans.length; ++i) {
						...
						if (
							...
							markers.push(
								span.marker.parent || span.marker,
							);
						}
					}
				}
	...
	for (var i$2 = 1; i$2 <= 12; i$2++) {
		keyNames[i$2 + 111] = keyNames[i$2 + 63235] =
			'F' + i$2;
	}
	...
	function moveCharLogically(line, ch, dir) {
		var target = skipExtendingChars(
			line.text,
			ch + dir,
			dir,
		);
		return target < 0 || target > line.text.length
			? null
			: target;
	}

	function moveLogically(line, start, dir) {
		var ch = moveCharLogically(
			line,
			start.ch,
			dir,
		);
		...
		if (visually) {
			...
			if (order) {
				...
				if (
					...
					var prep = prepareMeasureForLine(
						cm,
						lineObj,
					);
					ch =
						dir < 0 ? lineObj.text.length - 1 : 0;
					var targetTop = measureCharPrepared(
						cm,
						prep,
						ch,
					).top;
					ch = findFirst(
						function (ch) {
							return (
								measureCharPrepared(cm, prep, ch)
									.top == targetTop
							);
						},
						dir < 0 == (part.level == 1)
	...
	var commands = {
		...
		delWrappedLineLeft: function (cm) {
			return deleteNearSelection(
				...
				function (range) {
					var top =
						cm.charCoords(range.head, 'div').top +
						5;
			...
			return deleteNearSelection(
				...
				function (range) {
					...
					var rightPos = cm.coordsChar(
						{
							left:
								cm.display.lineDiv.offsetWidth +
								100,
							top: top,
						},
						'div',
		...
		goLineRight: function (cm) {
			return cm.extendSelectionsBy(function (
				range,
			) {
				var top =
					cm.cursorCoords(range.head, 'div').top +
					5;
	...
	function onKeyDown(e) {
		...
		if (
			e.target &&
			e.target != cm.display.input.getField()
		) {
			return;
		}
	...
	function onKeyPress(e) {
		...
		var ch = String.fromCharCode(
			charCode == null ? keyCode : charCode,
		);
	...
	PastClick.prototype.compare = function (
		time,
		pos,
		button,
	) {
	...
	function clickRepeat(pos, button) {
		...
		if (
			...
			lastClick &&
			lastClick.compare(now, pos, button)
		) {
	...
	function gutterEvent(cm, e, type, prevent) {
		...
		for (
			var i = 0;
			i < cm.display.gutterSpecs.length;
			++i
		) {
	...
	function defineOptions(CodeMirror) {
		...
		option(
			...
			false,
			function (cm, val) {
				return (cm.getInputField().spellcheck =
					val);
			},
			true,
		);
		option(
			...
			false,
			function (cm, val) {
				return (cm.getInputField().autocorrect =
					val);
			},
			true,
		);
		option(
			...
			false,
			function (cm, val) {
				return (cm.getInputField().autocapitalize =
					val);
			},
			true,
		);
		...
		option(
			...
			function (cm, val) {
				cm.display.gutters.style.left = val
					? compensateForHScroll(cm.display) +
						'px'
					: '0';
				cm.refresh();
			},
		...
		option('tabindex', null, function (cm, val) {
			return (cm.display.input.getField().tabIndex =
				val || '');
		});
	...
	function CodeMirror(place, options) {
		...
		var input = new CodeMirror.inputStyles[
			options.inputStyle
		](this);
		...
		if (
			(options.autofocus && !mobile) ||
			this.hasFocus()
		) {
			setTimeout(function () {
				if (
					this$1.hasFocus() &&
					!this$1.state.focused
				) {
					onFocus(this$1);
				}
			}, 20);
		...
		if (
			...
			getComputedStyle(display.lineDiv)
				.textRendering == 'optimizelegibility'
		) {
			display.lineDiv.style.textRendering =
				'auto';
		}
	}
	...
	function registerEventHandlers(cm) {
		...
		on(d.wrapper, 'scroll', function () {
			return (d.wrapper.scrollTop =
				d.wrapper.scrollLeft =
					0);
		});
	...
	function indentLine(cm, n, how, aggressive) {
		...
		if (how == 'prev') {
			...
			indentation =
				curSpace + cm.options.indentUnit;
		...
		if (cm.options.indentWithTabs) {
			for (
				var i = Math.floor(indentation / tabSize);
				i;
				--i
			) {
				pos += tabSize;
	...
	function applyTextInput(
		...
		if (paste && sel.ranges.length > 1) {
			if (
				...
				if (
					sel.ranges.length %
						lastCopied.text.length ==
					0
				) {
					multiPaste = [];
					for (
						var i = 0;
						i < lastCopied.text.length;
						i++
					) {
						multiPaste.push(
							doc.splitLines(lastCopied.text[i]),
						);
					}
				}
			} else if (
	...
	function triggerElectric(cm, inserted) {
		...
		for (
			...
			if (mode.electricChars) {
				for (
					var j = 0;
					j < mode.electricChars.length;
					j++
				) {
					if (
						inserted.indexOf(
							mode.electricChars.charAt(j),
						) > -1
					) {
						indented = indentLine(
							cm,
	...
	function addEditorMethods(CodeMirror) {
		...
		CodeMirror.prototype = {
			...
			lineAtHeight: function (height, mode) {
				...
				return lineAtHeight(
					this.doc,
					height + this.display.viewOffset,
				);
			},
			...
			findWordAt: function (pos) {
				...
				if (line) {
					...
					while (
						start > 0 &&
						check(line.charAt(start - 1))
					) {
						--start;
					}
					while (
						end < line.length &&
						check(line.charAt(end))
					) {
						++end;
					}
				}
			...
			scrollIntoView: methodOp(
				function (range, margin) {
					if (range == null) {
						...
						if (margin == null) {
							margin =
								this.options.cursorScrollMargin;
						}
			...
			refresh: methodOp(function () {
				...
				if (
					oldHeight == null ||
					Math.abs(
						oldHeight - textHeight(this.display),
					) > 0.5 ||
	...
	function findPosH(
		...
		function moveOnce(boundToLine) {
			var next;
			if (unit == 'codepoint') {
				var ch = lineObj.text.charCodeAt(
					pos.ch + (dir > 0 ? 0 : -1),
				);
				if (isNaN(ch)) {
					next = null;
		...
		if (unit == 'char' || unit == 'codepoint') {
			...
			var helper =
				doc.cm &&
				doc.cm.getHelper(pos, 'wordChars');
			for (var first = true; ; first = false) {
				if (dir < 0 && !moveOnce(!first)) {
					break;
				}
				var cur =
					lineObj.text.charAt(pos.ch) || '\n';
	...
	ContentEditableInput.prototype.init = function (
		display,
	) {
		var this$1 = this;
	...
	ContentEditableInput.prototype.screenReaderLabelChanged =
		function (label) {
	...
	ContentEditableInput.prototype.prepareSelection =
		function () {
			var result = prepareSelection(
				this.cm,
				false,
			);
	...
	ContentEditableInput.prototype.showSelection =
		function (info, takeFocus) {
	...
	ContentEditableInput.prototype.getSelection =
		function () {
			return this.cm.display.wrapper.ownerDocument.getSelection();
		};

	ContentEditableInput.prototype.showPrimarySelection =
		function () {
			...
			if (
				...
				curFocus &&
				!curFocus.bad &&
				cmp(minPos(curAnchor, curFocus), from) ==
					0 &&
			...
			var end =
				to.line < cm.display.viewTo &&
				posToDOM(cm, to);
			if (!end) {
				var measure =
					view[view.length - 1].measure;
	...
	ContentEditableInput.prototype.showMultipleSelections =
		function (info) {
			removeChildrenAndAdd(
	...
	ContentEditableInput.prototype.rememberSelection =
		function () {
			var sel = this.getSelection();
	...
	ContentEditableInput.prototype.selectionInEditor =
		function () {
			var sel = this.getSelection();
			if (!sel.rangeCount) {
				return false;
	...
	ContentEditableInput.prototype.focus =
		function () {
			if (
	...
	ContentEditableInput.prototype.blur =
		function () {
			this.div.blur();
		};
	ContentEditableInput.prototype.getField =
		function () {
			return this.div;
		};

	ContentEditableInput.prototype.supportsTouch =
		function () {
			return true;
		};

	ContentEditableInput.prototype.receivedFocus =
		function () {
			var this$1 = this;
	...
	ContentEditableInput.prototype.selectionChanged =
		function () {
			var sel = this.getSelection();
			return (
	...
	ContentEditableInput.prototype.pollSelection =
		function () {
			if (
				this.readDOMTimeout != null ||
				this.gracePeriod ||
	...
	ContentEditableInput.prototype.pollContent =
		function () {
			...
			if (
				to.ch ==
					getLine(cm.doc, to.line).text.length &&
				to.line < cm.lastLine()
			) {
				to = Pos(to.line + 1, 0);
			}
			...
			if (
				...
				fromLine = lineNo(
					display.view[fromIndex].line,
				);
				fromNode =
					display.view[fromIndex - 1].node
						.nextSibling;
			}
			...
			if (toIndex == display.view.length - 1) {
				...
				toLine =
					lineNo(display.view[toIndex + 1].line) -
					1;
				toNode =
					display.view[toIndex + 1].node
						.previousSibling;
			}
			...
			var newTop = newText[0],
				oldTop = oldText[0],
				maxCutFront = Math.min(
					newTop.length,
					oldTop.length,
				);
			while (
				cutFront < maxCutFront &&
				newTop.charCodeAt(cutFront) ==
			...
			var maxCutEnd = Math.min(
				newBot.length -
					(newText.length == 1 ? cutFront : 0),
				oldBot.length -
					(oldText.length == 1 ? cutFront : 0),
			);
			while (
				cutEnd < maxCutEnd &&
				newBot.charCodeAt(
					newBot.length - cutEnd - 1,
				) ==
					oldBot.charCodeAt(
	...
	ContentEditableInput.prototype.ensurePolled =
		function () {
			this.forceCompositionEnd();
		};
	ContentEditableInput.prototype.reset =
		function () {
			this.forceCompositionEnd();
		};
	ContentEditableInput.prototype.forceCompositionEnd =
		function () {
			if (!this.composing) {
				return;
			}
	...
	ContentEditableInput.prototype.readFromDOMSoon =
		function () {
			var this$1 = this;
	...
	ContentEditableInput.prototype.updateFromDOM =
		function () {
			var this$1 = this;
	...
	ContentEditableInput.prototype.setUneditable =
		function (node) {
			node.contentEditable = 'false';
		};

	ContentEditableInput.prototype.onKeyPress =
		function (e) {
	...
	ContentEditableInput.prototype.readOnlyChanged =
		function (val) {
	...
	function posToDOM(cm, pos) {
		...
		var result = nodeAndOffsetInLineMap(
			info.map,
			pos.ch,
			side,
		);
		result.offset =
			result.collapse == 'right'
				? result.end
				: result.start;
		return result;
	}
	...
	function domToPos(cm, node, offset) {
		...
		if (node == cm.display.lineDiv) {
			lineNode =
				cm.display.lineDiv.childNodes[offset];
			if (!lineNode) {
				return badPos(
					cm.clipPos(Pos(cm.display.viewTo - 1)),
					true,
				);
			}
			node = null;
	...
	function locateNodeInLineView(
		...
		if (node == wrapper) {
			...
			offset = 0;
			if (!node) {
				var line = lineView.rest
					? lst(lineView.rest)
					: lineView.line;
				return badPos(
					Pos(lineNo(line), line.text.length),
					bad,
				);
			}
		}
		...
		for (
			var before = topNode.previousSibling,
				dist$1 = offset;
			before;
			...
			if (found) {
				return badPos(
					Pos(found.line, found.ch + dist$1),
					bad,
				);
			} else {
	...
	TextareaInput.prototype.init = function (
		display,
	) {
		var this$1 = this;
	...
	TextareaInput.prototype.showSelection =
		function (drawn) {
			var cm = this.cm,
				display = cm.display;
			removeChildrenAndAdd(
	...
	TextareaInput.prototype.reset = function (
		typing,
	) {
		if (
	...
	TextareaInput.prototype.supportsTouch =
		function () {
			return false;
		};
	...
	TextareaInput.prototype.receivedFocus =
		function () {
			this.slowPoll();
		};
	...
	TextareaInput.prototype.ensurePolled =
		function () {
	...
	TextareaInput.prototype.onKeyPress =
		function () {
			if (ie && ie_version >= 9) {
				this.hasSelection = null;
			}
	...
	TextareaInput.prototype.onContextMenu =
		function (e) {
			var input = this,
				cm = input.cm,
				display = cm.display,
	...
	TextareaInput.prototype.readOnlyChanged =
		function (val) {
			if (!val) {
				this.reset();
			}
	...
	for (var prop in Doc.prototype) {
		if (
			...
			CodeMirror.prototype[prop] = (function (
				method,
			) {
				return function () {
					return method.apply(
